 ...
 * @desc		This is the core input handler of the Dunamis Framework
 * @package		Dunamis
 * @subpackage	Core
 * @author		@packageAuth@
 * @link		@packageLink@
 ...
 * @license		@packageLice@
 */
class DunInput extends DunObject
{
	 ...
	 * @since		1.0.3
	 */
	public function getVar( $item, $default = false, $src = 'request', $filter = 'string' )
	{
		if (! array_key_exists( $src, $this->src ) ) {
			// Throw an error for no src
			return $default;
		}
		
		if (! array_key_exists( $item, $this->src[$src] ) ) {
			return $default;
		}
	...
	/**
	 * Loads the data passed into the object
	 * @access		protected
	 * @version		@fileVers@
	 * @param		array		- $data: compiled data to send
	 * 
	 * @since		1.0.3
	 */
	protected function load( $data = array() )
	{
		foreach ( $this->src as $item => $x ) {
			if (! array_key_exists( $item, $data ) ) continue;
	...
	/**
	 * Method to clean a variable
	 * Taken from JFilterInput class in Joomla! 2.5.8
	 * Forked from the php input filter library by: Daniel Morris <dan@rootcube.com>
	 * Original Contributors: Gianpaolo Racca, Ghislain Picard, Marco Wandschneider, Chris Tobin and Andrew Eddie.
	 * 
	 * @access		public
	 * @version		@fileVers@
	 * @param		mixed		- $source: to be cleaned
	 * @param		string		- $type: the type of filter to apply
	 * 
	 * @return		mixed containing cleaned item
	 * @since		1.0.3 
	 */
